...
export const POST: APIRoute = async ({ request, locals, redirect }) => {
  ...
  if (!matchId || !playerId) {
    return redirect(`/app/matches/${matchId}?error=${encodeURIComponent('Chybí match_id nebo player_id.')}`, 303);
  }
  ...
  return redirect(`/app/matches/${matchId}?saved=stats#player-${playerId}`, 303);
};
